...
class Deck_Tests(unittest.TestCase):
	...
	def test_replace_card(self):
		# This test checks that if after popping the 1 of Diamonds and replacing it using the
	...
	def test_replace_card2(self):
		# Test to see if adding an existing card to a deck adds multiple list elements
		d = Deck()
		dd = Deck()
		...
		self.assertEqual(l_of_cards.count(tt), 1, "Testing that replace card didn't add existing element")
	...
	def test_deal_hand(self): # add assertEqual etc later
		...
		self.assertEqual(len(my_hand), 4, "Testing that full deck deals hand size of 4")
